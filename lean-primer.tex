...
\paragraph{What the system actually outputs.} A tactic block is only
\emph{sugar}: the \code{by} script elaborates to an ordinary \emph{term}, and it
is that term the kernel checks. You can print it. The trivial one is exactly what
you would guess---the tactic \code{rfl} built the term \code{Eq.refl (2 + 2)}:
...
\noindent And the induction above is not magic either: it elaborates to a use of
\code{Nat}'s recursor, with our two cases as its arguments (elided for brevity):
...
\noindent So ``Lean proved it'' means: a term was constructed---by hand, or by a
tactic on your behalf---and a small, fixed \textbf{kernel} (a few thousand lines)
re-checked that the term has the stated type. Automation and tactics live
\emph{outside} the kernel; a buggy tactic cannot produce a false theorem, because
whatever it emits must still pass the kernel (\Cref{fig:pipeline}). The trusted
base is the kernel alone, not the sprawling toolchain around it
\cite{demoura-lean4}. A proof may also contain \code{sorry}, an explicit hole
that admits any goal so you can sketch structure and fill gaps later---Lean warns
loudly while any remains.
...
\textbf{what you type}---the definition and a tactic proof:
