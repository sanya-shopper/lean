...
already performs---verifying that a term has the type it claims. Concretely:

\begin{lstlisting}
-- The proposition `2 + 2 = 4` is a type; `rfl` builds a term inhabiting it.
theorem two_plus_two : 2 + 2 = 4 := by rfl

-- `∀ n, n < n + 1` is a type (a function type); the proof is a function that,
-- given n, produces evidence. `omega` builds that term for us.
theorem succ_gt (n : Nat) : n < n + 1 := by omega

-- A property of our own function, with an existential witness:
theorem double_even (n : Nat) : ∃ k, double n = 2 * k :=
  ⟨n, by unfold double; omega⟩
\end{lstlisting}

\noindent What does the machine actually guarantee? Every proof, however it was
produced, is elaborated into a fully explicit term that is then re-checked by a
small, fixed \textbf{kernel}---a few thousand lines whose only job is to verify
that a term has its stated type. Automation, tactics, and syntactic conveniences
all live \emph{outside} the kernel; a buggy tactic cannot produce a false
theorem, because whatever it emits must still pass the kernel
(\Cref{fig:pipeline}). So ``Lean proved it'' means: a term was constructed and
the kernel confirmed it inhabits the proposition's type. The trusted base is the
kernel alone, not the sprawling toolchain around it \cite{demoura-lean4}. A
proof may also contain \code{sorry}, an explicit hole that admits any goal so you
can sketch structure and fill gaps later---Lean warns loudly while any remains.
...
theorem, both machine-checked in pure Lean (no library):
...
\noindent The proof is a textbook induction on $m$ that splits on the parity of
$\ell$ and steps down through the recurrence---exactly the \emph{Concrete
Mathematics} argument, now checked by the kernel rather than by a careful reader.
...
on real mathematics; it is also a heavy dependency, which is why the examples
here deliberately use none of it (core Lean plus its standard library,
\emph{Batteries}, suffices for algorithmic and bit-level work). Good next steps:
\emph{Theorem Proving in Lean~4} \cite{avigad-tpil4} for the proof language,
\emph{Functional Programming in Lean} \cite{christiansen-fpil} for the
programming side, and \emph{Mathematics in Lean} \cite{avigad-mil} for working
with mathlib. The companion \texttt{primer/} project holds every example above as
runnable, type-checked source.
